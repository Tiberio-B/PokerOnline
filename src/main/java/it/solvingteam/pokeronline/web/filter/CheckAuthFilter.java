package it.solvingteam.pokeronline.web.filter;
...
public class CheckAuthFilter implements Filter {
	...
	private static final String[] EXCLUDED_URLS = {"/HomepageServlet", "", "login.jsp","/css/","/js/"};
	private static final String[] ADMIN_ROLE_URLS = {"/utente/"};
	private static final String[] SPECIAL_PLAYER_ROLE_URLS = {"/tavolo/"};
	private static final String[] PLAYER_ROLE_URLS = {"/partita/"};
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!isInWhiteList) {
			Utente utenteInSession = (Utente)httpRequest.getSession().getAttribute("utente");
			...
			if(isPathForAdminRoleOnly(pathAttuale) && !utenteInSession.isAdmin()) {
				Utils.addError(httpRequest, "Non si è autorizzati alla navigazione richiesta");
	...
	private boolean isPathForAdminRoleOnly(String requestPath) {
		for (String urlPatternItem : ADMIN_ROLE_URLS) {
			if (requestPath.contains(urlPatternItem)) {
				return true;
			}
		}
	...
	private boolean isPathForSpecialPlayerRoleOnly(String requestPath) {
		for (String urlPatternItem : SPECIAL_PLAYER_ROLE_URLS) {
			if (requestPath.contains(urlPatternItem)) {
				return true;
			}
		}
	...
	private boolean isPathForPlayerOnly(String requestPath) {
		for (String urlPatternItem : PLAYER_ROLE_URLS) {
			if (requestPath.contains(urlPatternItem)) {
				return true;
			}
		}
